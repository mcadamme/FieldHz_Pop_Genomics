...
sig_levels <- list(0.01, 0.005, 0.001)
df <- data.frame()
...
getSig_QTL <- function(X, outfile){
  for (i in sig_levels){
    ...
    temp_df <- subset(X, p_lrt < i & gamma > 0.01)
...
BAP11A1_DD <- read.csv("BAP11A1_DD_all_snps_4.5.21.csv", header = T)#BC0805
str(BAP11A1_DD)
sub_BAP11A1_DD <- subset(BAP11A1_DD, allele_frequency > 0.0499)

BAP11A1_CL <- read.csv("BAP11A1_CL_all_snps_4.5.21.csv", header = T)#providence
str(BAP11A1_CL)
sub_BAP11A1_CL <- subset(BAP11A1_CL, allele_frequency > 0.0499)

DEO9A1_DD <- read.csv("DEO9A1_DD_all_snps_4.5.21.csv", header = T)#ObsII
str(DEO9A1_DD)
sub_DEO9A1_DD <- subset(DEO9A1_DD, allele_frequency > 0.0499)

DEO9A1_CL <- read.csv("DEO9A1_CL_all_snps_4.5.21.csv", header = T)#Obs
str(DEO9A1_CL)
sub_DEO9A1_CL <- subset(DEO9A1_CL, allele_frequency > 0.0499)
...
table(read.csv("sig_BAP11A1_DD.csv", header = T)[,20])
table(read.csv("sig_BAP11A1_CL.csv", header = T)[,20])
table(read.csv("sig_DEO9A1_DD.csv", header = T)[,20])
table(read.csv("sig_DEO9A1_CL.csv", header = T)[,20])

temp_df <- subset(DEO9A1_CL, p_lrt < 0.001)#sanity check for last table.  Only obs at this thresh has low allele freq.
...
For_SNPs <- data.frame(cbind(with_artPos$SnpName, with_artPos$Chr, with_artPos$radtag_position, with_artPos$p_lrt, with_artPos$gamma))
names(For_SNPs) <- c("SnpName", "Chr", "Pos", "Pval", "PIP")
For_SNPs$SnpName <- as.factor(For_SNPs$SnpName)
For_plot <- For_SNPs[,-5]
str(For_plot)#sanity check

SNPs <- subset(For_SNPs, Pval < sigLev, PIP > 0.01)
str(SNPs)

CMplot(For_plot[,-5], type="p",plot.type="m", LOG10=TRUE, threshold=NULL,file="jpg",memo=name,dpi=300,
       col=c("grey50","grey70"), pch = 21, cex = 0.8, highlight=SNPs[,1],
